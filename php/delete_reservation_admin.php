...
if(isset($_POST['password'])){
    ...
    $lieu = $_GET['lieu'];
    //Query pour voir si le mdp correspond
    $query = "SELECT * FROM Users WHERE mdp='$password' AND id_users ='$userid'";
    $result = mysqli_query($conn, $query);
    $count = mysqli_num_rows($result);
    //Query pour récupérer l'id destination
    $queryIdDest = "SELECT id_destination FROM Destinations WHERE nom='$lieu'";
    $resultIdDest = mysqli_query($conn,$queryIdDest);
    $rowIdDest= mysqli_fetch_assoc($resultIdDest);
    $id_destination=$rowIdDest['id_destination'];
    //Query pour récupérer le nombre de places
...
?>
    <div class="main-block2">
        <form method="post" action="#">
            <h5>Êtes vous sur de vouloir supprimer cette réservation ?<br>
